...
// fallback (404 or proxy). Global latency and fail-rate are applied by the
// caller via withLatencyAndFailures using the merged settings.
//
...
func NewRoutedHandler(cfg RoutedHandlerConfig) http.Handler {
	// Built-in endpoints (no latency/failures here; applied globally outside).
